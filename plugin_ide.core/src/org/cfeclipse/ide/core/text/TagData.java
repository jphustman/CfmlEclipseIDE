...
public class TagData implements IToken {
	 ...
	 */
	public TagData(String startPartitionType, String data, String midPartitionType, String endPartitionType,
			String tagName) {
	...
	private void parseData() {
		try {
			...
			for (int i = 0; i < fData.length(); i++) {
				if ((int) fData.charAt(i) <= 32 || fData.charAt(i) == '/' || fData.charAt(i) == '>') {
 ...
 * CVS LOG ====================================================================
 * 
 * $Log: TagData.java,v $ Revision 1.6 2005/09/30 06:20:45 rohanr2 Submitted by:
 * Jared Rypka-Hauer Reviewed by: Rob Rohan
 *
 * Revision 1.5 2005/07/25 01:29:38 rohanr2 trying to clean up the asserts and
 * the unused variable warnings... I just commented out the parts that had
 * unused variables and replaced all asserts with IllegalArgumentExceptions. I
 * only got through about half the code base.
 *
 * Revision 1.4 2005/01/27 01:37:25 smilligan put a band aid on the content
 * assist for attributes and tags. It will need to be properly sorted out
 * another time, but it basically works for now.
 *
 * Revision 1.3 2005/01/21 08:25:15 smilligan Re-implemented the partitioning in
 * a slightly more robust way.
 *
 * Revision 1.2 2005/01/19 02:50:11 smilligan Second commit of (now hopefully
 * working) rewritten partitioner.
 *
 * Revision 1.1 2005/01/09 02:06:33 smilligan First commit of rewritten
 * partitioner.
 *
 */