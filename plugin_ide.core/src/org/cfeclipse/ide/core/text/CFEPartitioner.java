 ...
 || fDeletedText.indexOf('/') >= 0
	...
	public IRegion documentChanged2(DocumentEvent e) {
        try {
            ...
            fScanner.setPartialRange(d, fReparseStart, d.getLength()
                    - fReparseStart, LangPartitionTypes.CF_SCRIPT.getId(), fReparseStart);
    ...
    private static String getStartPartitionName(String partitionType) {
        if (partitionType.startsWith("__htm_") 
            ...
            return LangPartitionTypes.FORM_START_TAG_BEGIN.getId();
        } else if (partitionType.startsWith("__table_")
                && !partitionType.startsWith("__table_end")) {
            return LangPartitionTypes.TABLE_START_TAG_BEGIN.getId();
        } else if (partitionType.startsWith("__cf_")
                && !partitionType.startsWith("__cf_end")) {
            return LangPartitionTypes.CF_START_TAG_BEGIN.getId();
        }
    ...
    private static String getEndPartitionName(String partitionType) {
        if (partitionType.startsWith("__htm_") 
            ...
            return LangPartitionTypes.HTM_START_TAG_END.getId();
                ...
                && !partitionType.startsWith("__form_end")) {
            return LangPartitionTypes.FORM_START_TAG_END.getId();
        } else if (partitionType.startsWith("__table_")
                && !partitionType.startsWith("__table_end")) {
            return LangPartitionTypes.TABLE_START_TAG_END.getId();
        } else if (partitionType.startsWith("__cf_")
                && !partitionType.startsWith("__cf_end")) {
            return LangPartitionTypes.CF_START_TAG_END.getId();
        }
